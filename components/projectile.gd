...
var visibility_notifier: VisibleOnScreenNotifier2D : set = set_visibility_notifer
...
func _get_configuration_warnings() -> PackedStringArray:
	if not hitbox or not visibility_notifier:
		return ["Projectiles need to have Hitbox and VisiblelOnScreenNotifier2D children nodes!"]
	else:
		return []
...
func set_visibility_notifer(value: VisibleOnScreenNotifier2D) -> void:
